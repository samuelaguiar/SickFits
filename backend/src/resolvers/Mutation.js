...
const setCookie = (userId, ctx) => {
  ...
  ctx.response.cookie("token", token, cookieOpt);
};

function authenticationCheck(id) {
  if (!id) {
...
const Mutations = {
  async createItem(parent, args, ctx, info) {
    authenticationCheck(ctx.request.userId);
  ...
  async deleteItem(parent, args, ctx, info) {
    ...
    if (!ownsItem && !hasPermissions) {
      throw new Error("You dont't have permission to do that!");
    }
    // 3. delete it!
  ...
  async resetPassword(parent, args, ctx, info) {
    ...
    // 2. Check if its a legint reset token
  ...
  async updatePermissions(parent, args, ctx, info) {
    ...
    authenticationCheck(id);
    // 2. Query the current user
    const currentUser = await ctx.db.query.user({ where: { id } }, info);
  ...
  async createOrder(parent, args, ctx, info) {
    ...
    const amount = user.cart.reduce(
      (tally, { item, quantity }) => tally + item.price * quantity,
      0
    );

    // 3. Create the sripe charge (turn token into $$$)
